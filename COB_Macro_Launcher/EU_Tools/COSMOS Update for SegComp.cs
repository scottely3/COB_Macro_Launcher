...
{
    internal class COSMOS_Update_for_SegComp
    {
        ...
        public static void Start()
        {
            TopMostDialogs.MessageBox(
                ...
                "WARNING: This launcher uses Excel data and fixed screen coordinates to update COSMOS SegComp.",
                "COSMOS Update for SegComp",
            ...
            try
            {
                ...
                for (int row = firstRowToProcess; row <= lastUsedRow; row++)
                {
                    SegCompRecord record = BuildRecord(activeSheet, row);
                    if (record.IsEmpty)
                    {
                        continue;
                    }
                    ...
                    WriteStatus(activeSheet, row, status);
        ...
        private static SegCompRecord BuildRecord(Excel.Worksheet worksheet, int row)
        {
            return new SegCompRecord
            {
                CarrierNumber = GetCellText(worksheet, row, CarrierNumberColumn),
                CobRowEffDate = GetCellText(worksheet, row, CobRowEffDateColumn),
                CobRowEndDate = GetCellText(worksheet, row, CobRowEndDateColumn),
                SubscriberNumber = GetCellText(worksheet, row, SubscriberNumberColumn),
                SiteCode = GetCellText(worksheet, row, SiteCodeColumn),
                Comment = GetCellText(worksheet, row, CommentColumn),
                SourceSystemGroupCode = GetCellText(worksheet, row, SourceSystemGroupCodeColumn),
                CobIndicator = GetCellText(worksheet, row, CobIndicatorColumn)
            };
        }

        private static string ProcessRecord(SegCompRecord record)
        {
        ...
        private static void RunCosmosFlow(SegCompRecord record)
        {
            ...
            Click(244, 574, 1, 210);
            TypeText((record.Comment ?? string.Empty) + new string(' ', 78));
            Pause(310);
        ...
        private static string GetCellText(Excel.Worksheet worksheet, int row, string columnLetter)
        {
            ...
            Excel.Range cell = worksheet.Cells[row, columnNumber] as Excel.Range;
            if (cell == null)
            {
                return string.Empty;
            }
        ...
        private sealed class SegCompRecord
        {
